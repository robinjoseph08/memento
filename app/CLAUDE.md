...
- Send `/api` traffic through the shared same-origin client and include the Session CSRF token on mutations. Direct production `/api` fetches are forbidden outside `api.ts`. Clear protected Query data whenever identity, Session, Invitation, Recovery, or offline transitions require it.
