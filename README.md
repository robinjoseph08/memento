...
The repository includes the deployable application foundation, first-browser Curator setup, the private Source album inbox, and private Event, Moment, and Loose item drafting: a React PWA, a Go API and in-process worker, PostgreSQL migrations, Caddy, and one production image. See [the product and architecture specification](docs/product-architecture-spec.md) and [canonical domain language](CONTEXT.md).
