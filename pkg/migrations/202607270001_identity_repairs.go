			...
			return db.RunInTx(ctx, nil, func(ctx context.Context, tx bun.Tx) error {
				statements := []string{
					...
					`CREATE TABLE media_repair_candidates (
						...
						previous_evidence jsonb NOT NULL DEFAULT '{}'::jsonb CHECK (jsonb_typeof(previous_evidence) = 'object'),
						candidate_evidence jsonb NOT NULL DEFAULT '{}'::jsonb CHECK (jsonb_typeof(candidate_evidence) = 'object'),
						face_anchor_evidence jsonb NOT NULL DEFAULT '[]'::jsonb CHECK (jsonb_typeof(face_anchor_evidence) = 'array'),
						conflict_evidence jsonb NOT NULL DEFAULT '[]'::jsonb CHECK (jsonb_typeof(conflict_evidence) = 'array'),
						created_at timestamptz NOT NULL,
						resolved_at timestamptz,
						resolved_by_person_id uuid REFERENCES people(id) ON DELETE RESTRICT,
