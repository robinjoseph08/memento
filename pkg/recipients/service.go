...
func (s *Service) CompleteOnboarding(ctx context.Context, actor setup.SessionActor, request OnboardingRequest) (OnboardingCompleteResponse, error) {
	...
	err := s.db.RunInTx(ctx, nil, func(ctx context.Context, tx bun.Tx) error {
		...
		if _, err := tx.NewRaw(`
			...
			FROM (
				SELECT publication_id, recipient_access_generation_id FROM current_audience_entitlements
				UNION
				SELECT publication_id, recipient_access_generation_id FROM current_loose_item_entitlements
			) AS entitlement
