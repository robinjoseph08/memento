...
- Define every Memento HTTP JSON request and response as a named exported Go struct with `snake_case` fields. Add frontend-visible contracts to `tygo.yaml`; do not expose persistence records as accidental wire contracts. Dependency adapters keep their provider-specific DTOs private.
