...
func TestConfirmedAttendanceBuildsExplainedProposalWithoutTrustingAdvisoryInputs(t *testing.T) {
	...
	review, err := f.service.ConfirmAttendance(ctx, f.actor, f.momentID, AttendanceRequest{PersonIDs: []string{f.people["present"].String(), f.people["both"].String(), f.people["attended"].String()}})
	require.NoError(t, err)
...
func TestOverridesSurviveRecalculationAndApprovedSnapshotsStayFixed(t *testing.T) {
	f := newAudienceFixture(t)
	ctx := context.Background()
	_, err := f.service.ConfirmAttendance(ctx, f.actor, f.momentID, AttendanceRequest{PersonIDs: []string{f.people["present"].String(), f.people["attended"].String()}})
	require.NoError(t, err)
	_, err = f.service.SetOverride(ctx, f.actor, targetMoment, f.momentID, OverrideRequest{RecipientPersonID: f.people["present"].String(), State: "excluded"})
	require.NoError(t, err)
	review, err := f.service.SetOverride(ctx, f.actor, targetMoment, f.momentID, OverrideRequest{RecipientPersonID: f.people["manual"].String(), State: "included"})
	require.NoError(t, err)
	...
	approved, err := f.service.Approve(ctx, f.actor, targetMoment, f.momentID)
	require.NoError(t, err)
	...
	_, err = f.service.ConfirmAttendance(ctx, f.actor, f.momentID, AttendanceRequest{PersonIDs: []string{f.people["both"].String()}})
	require.NoError(t, err)
	recalculated, err := f.service.Recalculate(ctx, f.actor, targetMoment, f.momentID)
	require.NoError(t, err)
...
func TestMomentAndLooseItemCanApproveExplicitlyEmptyCuratorOnlyAudiences(t *testing.T) {
	f := newAudienceFixture(t)
	ctx := context.Background()
	_, err := f.service.ConfirmAttendance(ctx, f.actor, f.momentID, AttendanceRequest{})
	require.NoError(t, err)
	moment, err := f.service.Approve(ctx, f.actor, targetMoment, f.momentID)
	require.NoError(t, err)
	assert.Equal(t, "Curator only", moment.Audience.Label)
	assert.Empty(t, moment.Audience.Recipients)
	_, err = f.service.Recalculate(ctx, f.actor, targetLoose, f.looseID)
	require.NoError(t, err)
	loose, err := f.service.Approve(ctx, f.actor, targetLoose, f.looseID)
	require.NoError(t, err)
...
func TestAttendanceAndOverrideFailuresLeaveReviewStateUnchanged(t *testing.T) {
	f := newAudienceFixture(t)
	ctx := context.Background()
	_, err := f.service.ConfirmAttendance(ctx, f.actor, f.momentID, AttendanceRequest{PersonIDs: []string{f.people["present"].String(), f.people["present"].String()}})
	assert.ErrorIs(t, err, ErrInvalid)
	_, err = f.service.ConfirmAttendance(ctx, f.actor, f.momentID, AttendanceRequest{PersonIDs: []string{uuid.NewString()}})
	assert.ErrorIs(t, err, ErrPersonUnavailable)
	_, err = f.service.SetOverride(ctx, f.actor, targetMoment, f.momentID, OverrideRequest{RecipientPersonID: f.people["suspended"].String(), State: "included"})
	assert.ErrorIs(t, err, ErrRecipientIneligible)
	_, err = f.service.Recalculate(ctx, f.actor, targetMoment, uuid.New())
