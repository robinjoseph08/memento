...
// readFaces lists every Immich face for the given Media Items. One failed
// read fails the whole refresh so cached associations are never half replaced.
func (m *Module) readFaces(ctx context.Context, items []faceRefreshItem) error {
...
// storeFaces replaces the cached associations of the given Media Items inside
// the caller's transaction. It does nothing when any item was refreshed by
// someone else since the faces were read, so a slower read never overwrites
// a newer one.
func storeFaces(ctx context.Context, tx bun.Tx, items []faceRefreshItem, now time.Time) error {
	...
	if len(mediaIDs) > 0 {
		...
		// Lock in source-ID order, the same order imports and syncs use, so a
		// refresh cannot deadlock against them on shared media.
		if err := tx.NewSelect().Model(&locked).Column("id").Where("id IN (?)", bun.List(mediaIDs)).OrderExpr("source_id COLLATE \"C\"").For("UPDATE").Scan(ctx); err != nil {
