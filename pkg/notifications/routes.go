...
// RegisterRoutes keeps previews, approval, delivery state, and the email
// diagnostic behind the Curator guard and a Person's own notifications behind the signed-in guard.
...
func RegisterRoutes(e *echo.Echo, module UseCases, diagnostic Diagnostic, requirePerson, requireCurator echo.MiddlewareFunc) {
	h := &handlers{module: module, diagnostic: diagnostic}
	e.GET("/api/curator/email", h.checkMail, requireCurator)
